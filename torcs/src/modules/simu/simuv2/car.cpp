...
SimCarUpdateForces(tCar *car)
{
	...
	if (v > 0.00001f) {
		Rv = R / v;
		if ((Rv * minv * SimDeltaTime) > v) { //what does this mean
			Rv = v * m / SimDeltaTime;
		}
	} else {
		Rv = 0.0f;
...
// wueli => add SimCarSetPos to have a standing car as obstacle
void
SimCarSetPos(tCar *car,  double x,  double y, double z, double phi)
{
	...
	car->DynGC.vel.ax = car->DynGCg.vel.ax = 0;
	car->DynGC.vel.ay = car->DynGCg.vel.ay = 0;
	car->DynGC.vel.az = car->DynGCg.vel.az = 0;
	car->DynGC.vel.x = car->DynGCg.vel.x = 0;
	car->DynGC.vel.y = car->DynGCg.vel.y = 0;
	car->DynGC.vel.z = car->DynGCg.vel.z = 0;
